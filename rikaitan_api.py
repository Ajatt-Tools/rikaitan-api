...
RIKAITAN_API_NATIVE_MESSAGING_VERSION = 1
...
class RequestHandler(http.server.BaseHTTPRequestHandler):
    def do_POST(self) -> None:  # noqa: N802
        ...
        if path in ["serverVersion", ""]:
            send_response(self, 200, "application/json", json.dumps({"version": RIKAITAN_API_NATIVE_MESSAGING_VERSION}))
            return

        send_message({"action": path, "params": params, "body": body})
        rikaitan_response = get_message()

        send_response(self, rikaitan_response["responseStatusCode"], "application/json", json.dumps(rikaitan_response["data"], ensure_ascii = False))
