...
# generate manifest
print()
print(f"Using default Rikaitan extension ID for {browser}.")
...
script_path = os.path.join(DIR, "rikaitan_api.py")
if platform_data["platform"] == "windows":
    bat_path = os.path.join(DIR, "rikaitan_api.bat")
...
if platform_data["platform"] == "mac":
    script_path = os.path.join(manifest_install_data["path"], "rikaitan_api.py")
    try:
        shutil.copy(os.path.join(DIR, "rikaitan_api.py"), script_path)
        print(f'File copied from {os.path.join(DIR, "rikaitan_api.py")} to {script_path}')
